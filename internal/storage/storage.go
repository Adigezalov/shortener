...
type URLStorage interface {
	...
	// Возвращает ID и признак того, был ли URL уже в хранилище
...
// Storage реализует хранилище URL с опциональным сохранением в файл
type Storage struct {
...
// Возвращает ID и признак того, был ли URL уже в хранилище
func (s *Storage) Add(id string, url string) (string, bool) {
	s.mu.Lock()
	defer s.mu.Unlock()

	// Проверяем, есть ли уже такой URL
	if existingID, found := s.urlToID[url]; found {
		return existingID, true // URL уже существует, возвращаем его ID
	}
...
// FindByOriginalURL ищет ID по оригинальному URL
func (s *Storage) FindByOriginalURL(url string) (string, bool) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Storage) writeBatch(records []models.URLRecord) error {
